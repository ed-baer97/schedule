...
| `auto_scheduler.py` | CP-SAT «Заполнить день» (остаток часов, слоты одного дня, другие дни якоря; `max_lesson` — не трогать более поздние уроки; Maximize наполнения, затем pack_gaps и soft; запись только выбранного дня); «лесенка» по учителю — first-fit + relocating, без DFS-перекладки смены; `repair_iter` — residual solver; ячейки только через `ScheduleService` |
| `schedule_solver.py` | residual + OR-Tools CP-SAT на фактах из loader; `day_of_week` — remaining hours, AtMostOne, Maximize наполнения дня, затем pack_gaps/early_rooms; `max_lesson` режет слоты и окно удаления; без дня — недельная пересборка (ExactlyOne); якорение двоек ≥4 ч (`freeze_policy`); ветвление hardest-first; веса из `ScheduleSettings` / `app.domain.preferences`; `y` только для пула ≤4; кабинеты после Solve (`_assign_rooms_to_chosen`); SAT: уроков в слоте ≤ сумма вместимостей |
