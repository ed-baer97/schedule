...
class AutoScheduler:
    ...
    def cp_sat_schedule_shift_iter(
        ...
        ``preserve_existing`` — мягкий дозапол (не удаляет стоящие ячейки).
        ...
        load_msg = (
            "Дозаполнение: загрузка классов и остатка часов…"
            if preserve_existing
            else "Загрузка классов и нагрузки…"
        )
        yield {
            "type": "progress",
            "current": 2,
