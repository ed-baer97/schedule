       ...
       └─ frontend/src/api            единственные fetch к /api
            ...
            app/services               use-case + SQL, school_id, DTO наружу
                 ...
                 └─ app/models          таблицы; @property без SQL
                      └─ migrations     Alembic (head: 17classroom_subgroup_only)
...
│   ├── domain/               # чистые предикаты (слот, кабинеты, сетка смены, дни, уровень, assist)
│   ├── models/               # сущности БД
...
│       ├── auth/             # AuthContext
│       ├── components/       # сетка: TeacherRemainingTip, TeacherDayGrid, ScheduleMinimap
│       ├── layouts/
│       ├── pages/            # только UI; HTTP только через api/
...
|------|-----------------|
| `frontend/src/pages/`, `components/`, `layouts/` | UI и маршруты; поллинг jobs в `AppLayout`; подсказки сетки в компонентах |
| `frontend/src/api/` | типы + fetch к `/api` (единственное место `fetch`) |
...
4. **Один write-path на агрегат**
   ...
   - `ScheduleCell` — только `ScheduleService` (`insert_cell` / `apply_placements` / `create_cell` / `move_cell` / `swap_classrooms` / `reposition_cell` / `delete_cell` / `delete_cells` / `clear_schedule`); авто и солвер делегируют сюда.
...
| `Classroom` | номер, вместимость; предметы через M2M `classroom_subjects` (пул), `is_exclusive` (только помеченные предметы; пустые теги = общий), `school_level` (`NULL` = общий / НШ / ОШ), `subgroup_only` (только подгруппы) |
| `Subject` | название, цвет; `subject_group` (`natural_math` / `humanities` / `languages` / `practical`); `requires_fixed_classroom`; обратная связь `classrooms` через ту же таблицу |
| `Shift`, `ShiftLessonTime` | смена и звонки; `class_hour_lessons_count` — короче сетка в день классного часа |
...
| `/api/schedule` | сетка (`teacher_remaining` — непроставленные часы учителей), ячейки, настройки; `GET …/teacher-day` — день учителя по сменам; `POST …/auto` (`day_of_week` обязателен, `max_lesson` — не ставить уроки позже N) и `POST …/repair` → `202` + `job_id`; `POST …/explain` — факты валидатора + текст Qwen; `POST …/assist` — фраза → веса и проверенные сдвиги |
...
| `schedule/` (`schedule_service` re-export) | сетка (queries: `get_grid`, `teacher_day`, `teacher_remaining`), ячейки (commands, единственный write-path, включая `swap_classrooms`), настройки |
| `dto.py` | общие DTO на границе сервисов |
| `errors.py` | `ServiceError` / `NotFoundError` / `BadRequestError` / `ConflictError` / `ValidationConflict` (HTTP в `backend/http_errors.py`) |
...
| `auto_scheduler.py` | CP-SAT «Заполнить день» (остаток часов, слоты одного дня, другие дни якоря; `max_lesson` — не трогать более поздние уроки; `preserve_existing` — мягкий дозапол без удаления стоящих; Maximize наполнения, затем pack_gaps и soft; запись только выбранного дня); «лесенка» по учителю — first-fit + relocating, без DFS-перекладки смены; `repair_iter` — residual solver; ячейки только через `ScheduleService` |
| `schedule_solver.py` | residual + OR-Tools CP-SAT на фактах из loader; `day_of_week` — remaining hours, AtMostOne, Maximize наполнения дня, затем pack_gaps/early_rooms; `preserve_existing` — якоря из уже стоящих ячеек, только insert; `max_lesson` режет слоты и окно удаления; без дня — недельная пересборка (ExactlyOne); якорение двоек ≥4 ч (`freeze_policy`); ветвление hardest-first; веса из `ScheduleSettings` / `app.domain.preferences`; `y` только для пула ≤4; кабинеты после Solve (`_assign_rooms_to_chosen`); SAT: уроков в слоте ≤ сумма вместимостей |
...
| `shift_grid.py` | `lessons_count_on_day` / `lesson_end_exclusive` / `capped_lesson_end_exclusive` / `weekly_slot_count` (день классного часа короче; лимит «не позже N-го») |
| `schedule_facts.py` | `UnitFact` / `SlotFact` / `BusySlotFact` |
| `schedule_rules.py` | слот, подгруппы, занятость, лимиты дня |
| `classroom_rules.py` | `room_has_subject` / `room_allows*` / `room_allows_subgroup` / `placement_cost` / `candidate_rooms_for` |
...
Фронт: `frontend/src/api/*` — единственные HTTP-вызовы (`health.ts`, jobs в `schedule.ts`; `import.ts` — raw `fetch` для FormData); страницы и компоненты (`TeacherRemainingTip`, `AppLayout`) только вызывают эти модули. `frontend/src/domain/` — `SchoolLevel`, дни, `groupsCanShareSlot`, `secondHourIsSplit`, `assignmentCanJoinSlot`, `roomHasSubject`, `roomAllows` / `roomAllowsSubject` / `roomAllowsSubgroup` / `roomFreeAtSlot`.

CP-SAT и циклы предметов: соседние уроки разных предметов одной группы (`natural_math` / `humanities` / `languages` / `practical`) штрафуются в пакете pack_gaps (`pref_same_group_adjacent`), вместе с окнами и сдвоенными. Сдвоенный один и тот же предмет не штрафуется. Миграция: `18subject_group` (вместо `subjects.difficulty`). Дальше: `15class_hour_lessons` (`shifts.class_hour_lessons_count`), `16requires_subgroup` (колонка на предмете, снята в 17), `17classroom_subgroup_only`. Чеклист колонок при старте API — `backend/database.py` (`subjects.subject_group`, `classrooms.subgroup_only`, `shifts.class_hour_lessons_count`).
...
- Домен: `test_schedule_rules`, `test_classroom_rules`, `test_shift_grid`, `test_preferences`, `test_pair_epochs`, `test_assist_intent`, `test_schedule_solver_stages`
- API: `test_api_health`, `test_api_crud`, `test_api_schedule`, `test_api_jobs`, `test_excel_import`, `test_schedule_excel`, `test_report_export`
- Прочее: `test_schema_util`
